    ...
    console.log(
      colors.green(
        `SPIN: ${r1} ${r2} ${r3} - Bet: X${bet} Pay ${pay}, Coins : ${numeral(
          ...
          "$(0.000a)"
        )}, Shields: ${shields}, Spins : ${spins} \t| Raid :${raid.name
        }(${numeral(raid.coins).format("$(0.000a)")}) H: ${this.spinCountFromAttack
...
*/
  ...
  async claimTodayRewardsV1() {
    ...
    } = await axios.get("https://raw.githubusercontent.com/blackundo/get-fb-app-token/refs/heads/main/spinfree.json");
  ...
  async upgrade(spinResult) {
    ...
    for (const item of priority) {
      //undo edit 
      if (villageLevel < 6) {
        if (this.priorityUpgrade && item != this.priorityUpgrade && spinResult[this.priorityUpgrade] < 4) continue;
      } else {
      ...
      const printMessage = `Upgrade Result: Village ${village + 1} \t Farm: ${Farm} \t House: ${House} \t Statue: ${Statue} \t Crop: ${Crop} \t Ship: ${Ship} \t | Cost ${deltaCoins}`;
